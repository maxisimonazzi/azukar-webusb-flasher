...
test('starter top is azukar_lab and ships uart_tx as a second file', () => {
  assert.equal(BLINKY_TOP, 'azukar_lab')
  assert.deepEqual(
    FPGA_STARTER.map((f) => f.name),
    ['azukar_lab.v', 'uart_tx.v'],
  )
  assert.equal(FPGA_STARTER.every((f) => f.open), true)
})

test('azukar_lab instantiates uart_tx on pin TX at 115200', () => {
  assert.match(BLINKY_VERILOG, /module azukar_lab/)
